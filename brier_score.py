...
=============================================================
Calcule le Brier Score d'une soumission passée en la
...
Usage :
  python brier_score.py \
    --submission  submission_2025.csv \
    --season      2025 \
...
def load_results(data_dir, season):
    """
    ...
    m_season = m_results[m_results["Season"] == season].copy()
    w_season = w_results[w_results["Season"] == season].copy()

    print(f"  Matchs hommes trouvés  ({season}) : {len(m_season)}")
    print(f"  Matchs femmes trouvés  ({season}) : {len(w_season)}")

    if len(m_season) == 0 and len(w_season) == 0:
        raise ValueError(
            f"Aucun résultat trouvé pour la saison {season}. "
...
def build_ground_truth(results_df, season):
    """
...
def compute_brier_score(submission, ground_truth, season):
    """
    ...
    if matched == 0:
        raise ValueError(
            f"Aucun match réel trouvé dans la soumission pour les saisons {season}.\n"
...
def evaluate(submission_path, season, details, calibration, goodbads, data_dir):
    print("=" * 55)
    print(f"  Évaluation Brier Score — Saison {season}")
    print("=" * 55)

    print("\n[1/3] Chargement des données...")
    results_df = load_results(data_dir, season)
    submission = load_submission(submission_path)

    print("\n[2/3] Construction de la vérité terrain...")
    ground_truth = build_ground_truth(results_df, season)
    
    print("\n[3/3] Calcul du Brier Score...")
    brier_score, details_df = compute_brier_score(submission, ground_truth, season)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Année du tournoi à évaluer (ex: 2025)"
    )
    parser.add_argument(
        "--details",
        action="store_true",
