...
def train_model_walk_forward(train_df, target_seasons=range(2022, 2027), n_splits=5):
    """
    Pour chaque season cible S, entraîne un modèle CV sur 2003..S inclus.
    Les OOF preds sont calculées uniquement sur la season S.

    Retourne :
        season_models : dict {season: [models_folds]}
        oof_preds_df  : DataFrame avec colonnes [season, y_true, y_pred]
    """
    ...
    for season in target_seasons:
        print(f"\n{'#'*50}")
        print(f"# SEASON {season}  —  train sur ≤ {season} inclus")
        print(f"{'#'*50}")
...
sub_stage1 = generate_submission(test_stage1, models, 'stage1_sub.csv', 0.0010)
print(f"Stage 1 saved: stage1_sub.csv")
sub_stage2 = generate_submission(test_stage2, models, 'stage2_sub.csv', 0.0010)
